...
$outputDir = Join-Path $repoRoot 'build\mcp-device-smoke'
...
$placeCommandsLog = Join-Path $outputDir 'place-commands.json'
$blocksLog = Join-Path $outputDir 'blocks.json'
$extraModsLog = Join-Path $outputDir 'extra-mods.txt'
$nextRequestId = 1

New-Item -ItemType Directory -Force -Path $outputDir | Out-Null
Remove-Item -LiteralPath $stdoutLog, $stderrLog, $toolsLog, $pingLog, $initializeLog, $playerInfoLog, $placeCommandsLog, $blocksLog, $extraModsLog -Force -ErrorAction SilentlyContinue
...
if (-not (Test-Path -LiteralPath $worldDir)) {
    throw "MCP device smoke requires an existing local save at $worldDir. Create a singleplayer test world with folder/name '$WorldName', then rerun this script."
}
...
try {
    ...
    if ($null -eq $pingResponse) {
        throw "MCP device smoke timed out before ping succeeded at $McpUrl. Last ping error: $lastPingError. Logs: $clientLog, $stdoutLog, $stderrLog"
    }
    ...
    $initializeResponse = Invoke-McpRequest -Method 'initialize' -Params @{
        ...
        capabilities = @{}
        clientInfo = @{
            name = 'neoopencomputers-device-smoke'
            version = '1.0.0'
        }
    }
    ...
    if ($null -eq $playerInfo -or $null -eq $playerInfo.dimension) {
        throw "MCP device smoke reached MCP but not an in-world player for '$WorldName'. Last error: $lastPlayerInfoError. Logs: $clientLog, $stdoutLog, $stderrLog"
    }
    ...
    Get-McpToolText $placeResponse | Out-Null
    $placeResponse | ConvertTo-Json -Depth 50 | Set-Content -LiteralPath $placeCommandsLog -Encoding UTF8
    ...
    $blockScan = $blockText | ConvertFrom-Json
    $scannedBlockIds = @($blockScan.blocks.PSObject.Properties | ForEach-Object { $_.Name })
    $expectedBlockIds = @(
        'neoopencomputers:computer_case_tier1',
        'neoopencomputers:screen_tier1',
        'neoopencomputers:keyboard',
    ...
    if ($missingBlockIds.Count -gt 0) {
        throw "MCP device smoke placed layout but block scan missed: $($missingBlockIds -join ', '). Evidence: $blocksLog"
    }
