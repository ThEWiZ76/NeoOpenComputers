...
public final class DisassemblerBlockEntityRenderer implements BlockEntityRenderer<DisassemblerBlockEntity> {
    ...
    private static final ResourceLocation TOP_ON_TEXTURE =
        ResourceLocation.fromNamespaceAndPath(NeoOpenComputers.MODID, "block/overlay/disassembler_top_on");
    private static final ResourceLocation SIDE_ON_TEXTURE =
        ResourceLocation.fromNamespaceAndPath(NeoOpenComputers.MODID, "block/overlay/disassembler_side_on");

    public DisassemblerBlockEntityRenderer(final BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(
        final DisassemblerBlockEntity disassembler,
        final float partialTick,
        final PoseStack poseStack,
        final MultiBufferSource bufferSource,
        final int packedLight,
        final int packedOverlay) {
        if (!disassembler.isVisuallyActive()) {
            return;
        }
    ...
    private static void renderSideOverlays(final PoseStack poseStack, final MultiBufferSource bufferSource, final int packedLight) {
        renderSideOverlay(poseStack, bufferSource, Direction.NORTH, packedLight);
        renderSideOverlay(poseStack, bufferSource, Direction.EAST, packedLight);
        renderSideOverlay(poseStack, bufferSource, Direction.SOUTH, packedLight);
        renderSideOverlay(poseStack, bufferSource, Direction.WEST, packedLight);
    }
