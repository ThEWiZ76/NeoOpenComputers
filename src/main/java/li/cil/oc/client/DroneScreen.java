...
import li.cil.oc.common.network.DroneControlPayload;
...
public class DroneScreen extends AbstractContainerScreen<DroneMenu> {
    ...
    public static final ResourceLocation DRONE_TEXTURE = ComputerCaseScreen.COMPUTER_TEXTURE;
    ...
    private static final int STATUS_CONTROL_Y = 51;
    private static final int STATUS_CONTROL_SIZE = 18;
    private static final Component SCREEN_TITLE = Component.translatable("gui.neoopencomputers.drone.title");

    public DroneScreen(final DroneMenu menu, final Inventory playerInventory, final Component title) {
    ...
    protected void renderBg(final GuiGraphics guiGraphics, final float partialTick, final int mouseX, final int mouseY) {
        ...
        guiGraphics.blit(DRONE_TEXTURE, left, top, 0, 0, imageWidth, imageHeight);
        final int tier = menu.droneTier();
        for (int slot = 0; slot < DroneMenu.droneSlotCountForTier(tier); slot++) {
            ComputerCaseScreen.drawSlot(
                guiGraphics,
                left + DroneMenu.droneSlotX(tier, slot) - 1,
                top + DroneMenu.droneSlotY(tier, slot) - 1,
    ...
    public boolean mouseClicked(final double mouseX, final double mouseY, final int button) {
        if (button == 0 && statusControlAt((int) mouseX, (int) mouseY, leftPos, topPos)) {
            PacketDistributor.sendToServer(controlPayload(menu, statusControlAction(menu.droneState())));
            return true;
        }
    ...
    public static int droneSlotAt(final int mouseX, final int mouseY, final int left, final int top, final int tier) {
        for (int slot = 0; slot < DroneMenu.droneSlotCountForTier(tier); slot++) {
            final int x = left + DroneMenu.droneSlotX(tier, slot);
            final int y = top + DroneMenu.droneSlotY(tier, slot);
    ...
    static DroneControlPayload controlPayload(final DroneMenu menu, final int action) {
        return new DroneControlPayload(menu.containerId, action);
    }

    static int statusControlAction(final int state) {
        return state == DroneMenu.STATE_RUNNING ? RackControlPayload.STOP : RackControlPayload.START;
    }
