...
public final class RelayBlockEntityRenderer implements BlockEntityRenderer<RelayBlockEntity> {
    ...
    private static final ResourceLocation ACTIVITY_TEXTURE =
        ResourceLocation.fromNamespaceAndPath(NeoOpenComputers.MODID, "block/overlay/switch_side_on");

    public RelayBlockEntityRenderer(final BlockEntityRendererProvider.Context context) {
    }
    ...
    public void render(
        ...
        final float alpha = (float) Mth.clamp(relay.visualActivity(), 0D, 1D);
        if (alpha <= 0F) {
            return;
        }
    ...
    private static void renderActivityOverlays(
        ...
        renderSideOverlay(Direction.NORTH, consumer, pose, alpha, packedLight);
        renderSideOverlay(Direction.SOUTH, consumer, pose, alpha, packedLight);
        renderSideOverlay(Direction.EAST, consumer, pose, alpha, packedLight);
        renderSideOverlay(Direction.WEST, consumer, pose, alpha, packedLight);
    }
