...
public final class WirelessNetworkCardHandler {
    ...
    public static void register() {
        NeoForge.EVENT_BUS.addListener(WirelessNetworkCardHandler::onRobotMove);
    }
    ...
    static void onRobotMove(final RobotMoveEvent.Post event) {
        ...
        for (final Node node : machine.node().reachableNodes()) {
            if (node != null && node.host() instanceof WirelessNetworkCardEnvironment card) {
                Network.updateWirelessNetwork(card);
            }
        }
    }
}
