...
public class PowerConverterBlock extends Block implements EntityBlock {
    public static final MapCodec<PowerConverterBlock> CODEC = simpleCodec(PowerConverterBlock::new);

    public PowerConverterBlock(final BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    protected void onRemove(final BlockState state, final Level level, final BlockPos pos, final BlockState newState, final boolean movedByPiston) {
        if (!state.is(newState.getBlock()) && level.getBlockEntity(pos) instanceof PowerConverterBlockEntity converter) {
            converter.removeNode();
        }
