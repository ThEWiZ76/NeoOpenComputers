...
public class GeolyzerBlock extends Block implements EntityBlock {
    public static final MapCodec<GeolyzerBlock> CODEC = simpleCodec(GeolyzerBlock::new);

    public GeolyzerBlock(final BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public BlockEntity newBlockEntity(final BlockPos pos, final BlockState state) {
        return new GeolyzerBlockEntity(pos, state);
    }
    ...
    protected void onRemove(final BlockState state, final Level level, final BlockPos pos, final BlockState newState, final boolean movedByPiston) {
        if (!state.is(newState.getBlock()) && level.getBlockEntity(pos) instanceof GeolyzerBlockEntity geolyzer) {
            geolyzer.removeNode();
        }
