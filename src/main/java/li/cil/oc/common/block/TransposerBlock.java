...
public class TransposerBlock extends Block implements EntityBlock {
    public static final MapCodec<TransposerBlock> CODEC = simpleCodec(TransposerBlock::new);

    public TransposerBlock(final BlockBehaviour.Properties properties) {
        super(properties);
    }
    ...
    public BlockEntity newBlockEntity(final BlockPos pos, final BlockState state) {
        return new TransposerBlockEntity(pos, state);
    }
    ...
    protected void onRemove(final BlockState state, final Level level, final BlockPos pos, final BlockState newState, final boolean movedByPiston) {
        if (!state.is(newState.getBlock()) && level.getBlockEntity(pos) instanceof TransposerBlockEntity transposer) {
            transposer.removeNode();
        }
