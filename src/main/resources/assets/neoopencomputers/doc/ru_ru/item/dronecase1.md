...
Их ограничения компенсируются скоростью передвижения и низкими энергозатратами. Они хорошо подходят для переноса небольших количеств предметов и разведки. Использование дрона вместе с роботом дает большие возможности, где робот выполняет тяжелую работу, а дрон предоставляет информацию об окружении и переносит предметы.

Как и микроконтроллеры, дроны могут быть запрограммированы только с помощью [EEPROM](eeprom.md). Соответственно, [EEPROM](eeprom.md) может быть изменен объединением дрона с другим [EEPROM](eeprom.md) на верстаке; старый [EEPROM](eeprom.md) при этом будет возвращен игроку.
