...
Корпус микроконтроллера используется для создания микроконтроллеров в [сборщике](../block/assembler.md). Микроконтроллеры - это очень примитивные [компьютеры](../general/computer.md). Они содержат только очень ограниченный набор компонентов и используются в специфических задачах: реагирование на сигнал красного камня или обработка сетевых сообщений.

Они не имеют файловой системы и программируются только с помощью [EEPROM](eeprom.md). Он может быть заменен на другой, если объединить микроконтроллер с другим чипом. При этом старый [EEPROM](eeprom.md) вернется в ваш инвентарь.
